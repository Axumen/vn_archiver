...
def _make_conn():
    ...
    conn.execute(
        "CREATE TABLE build (build_id INTEGER PRIMARY KEY, vn_id INTEGER NOT NULL, version TEXT, release_type TEXT, language TEXT, target_platform TEXT)"
    )
...
def test_mirror_metadata_for_rebuild_uses_file_build_file_table(tmp_path, monkeypatch):
    conn = _make_conn()
    conn.execute("INSERT INTO vn (vn_id, title) VALUES (1, 'Example VN')")
    conn.execute("INSERT INTO build (build_id, vn_id, version) VALUES (7, 1, '1.0')")
